...
imported directly -- it is always run as a child process. This ensures the
tests remain valid even if the tool is rewritten in another language.
...
By default, the cache is stored in a temporary directory under `/tmp`
that is shared across all xdist workers within a run but not preserved
across runs. To persist the cache across runs, set `SKOPEO_CACHE_DIR`:
